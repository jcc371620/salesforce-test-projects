...
class SalesforceClient: # 连接 Salesforce 的客户端类，封装了连接逻辑和查询方法
    ...
    def connect(self): 
        ...
        # 发送 POST 请求获取 Access Token，并处理响应。使用 requests 库发送 POST 请求到 token_url，携带上面构造的 payload。设置 timeout=30 来避免请求长时间挂起。如果响应状态码是 200，说明授权成功，我们从响应中提取 instance_url 和 access_token，然后用它们来初始化 Salesforce 实例。这样我们就绕过了 SOAP 登录限制，直接拿到了一个可以用来访问 Salesforce API 的 Token。如果授权失败，我们打印错误信息并返回 None。如果在网络连接过程中发生异常（比如超时、连接错误等），我们捕获异常并打印错误信息。
        try:
